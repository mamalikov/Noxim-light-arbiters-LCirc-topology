#include "Selection_DISTANCE_MAX.h"
#include "../GlobalParams.h"
#include <cmath>
#include <climits>
#include <vector>
SelectionStrategiesRegister Selection_DISTANCE_MAX::selectionStrategiesRegister("DISTANCE_MAX", getInstance());

Selection_DISTANCE_MAX * Selection_DISTANCE_MAX::selection_DISTANCE_MAX = 0;

Selection_DISTANCE_MAX * Selection_DISTANCE_MAX::getInstance() {
	if ( selection_DISTANCE_MAX == 0 )
		selection_DISTANCE_MAX = new Selection_DISTANCE_MAX();
    
	return selection_DISTANCE_MAX;
}

int Selection_DISTANCE_MAX::apply(Router * router, const vector < int >&directions, const RouteData & route_data){
    assert(directions.size()!=0);
    // For direction selection, use random (same as RANDOM)
    return directions[rand() % directions.size()];
}

void Selection_DISTANCE_MAX::perCycleUpdate(Router * router){ }

int Selection_DISTANCE_MAX::calculateRemainingDistance(int current_id, int dst_id) {
...
int Selection_DISTANCE_MAX::arbitrate(Router * router, const vector<pair<int,int> >& reservations) {
    assert(reservations.size() != 0);
    
    int max_distance = -1;
    vector<int> best_indices;
    
    // Find flits with maximum remaining distance
    ...
    // Return random among best (maximum distance)
