...
// LCirculant topology (circulant graph laid out on a grid)
#define TOPOLOGY_LCIRCULANT    "LCIRCULANT"
...
struct GlobalParams {
    ...
    // LCirculant topology parameters
    // Input parameters: S in [1,6], d, k (all integers)
    static int lcirc_S;
    static int lcirc_d;
    static int lcirc_k;
    // Derived parameters (computed from S,d,k according to LCirculant formulas)
    static int lcirc_N;   // number of nodes
    static int lcirc_s1;  // first circulant step
    static int lcirc_s2;  // second circulant step
    static int lcirc_a;   // grid width  (X dimension) used only for placement/visualization
    static int lcirc_b;   // grid height (Y dimension)
    // LCirculant base vectors a*, b*, and their sum (precomputed once)
    static int lcirc_ax;
    static int lcirc_ay;
    static int lcirc_bx;
    static int lcirc_by;
    static int lcirc_abx;
    static int lcirc_aby;
