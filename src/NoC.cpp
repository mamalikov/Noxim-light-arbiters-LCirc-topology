...
void NoC::buildLCirculant()
{
    ...
    // Basic sanity checks and debug print for LCirculant topology
    std::cout << "[LCIRC] topology parameters: "
              << "S=" << GlobalParams::lcirc_S
              << " N=" << N
              << " s1=" << s1
              << " s2=" << s2
              << " a=" << GlobalParams::lcirc_a
              << " b=" << GlobalParams::lcirc_b
              << std::endl;

    if (N <= 0 || s1 == 0 || s2 == 0) {
        std::cerr << "[LCIRC] ERROR: invalid LCirculant parameters, aborting buildLCirculant()" << std::endl;
        sc_stop();
        return;
    }

    // Allocate LCirculant link signals: one set per node and per direction
    lcirc_flit_link  = new sc_signal<Flit>*[N];
    lcirc_req_link   = new sc_signal<bool>*[N];
    lcirc_ack_link   = new sc_signal<bool>*[N];
    lcirc_buffer_full_status_link = new sc_signal<TBufferFullStatus>*[N];
    lcirc_free_slots_link = new sc_signal<int>*[N];
    lcirc_nop_link   = new sc_signal<NoP_data>*[N];

	std::cout << "Allocation completed" << std::endl;
    for (int i = 0; i < N; i++) {
        lcirc_flit_link[i]  = new sc_signal<Flit>[DIRECTIONS];
        lcirc_req_link[i]   = new sc_signal<bool>[DIRECTIONS];
        lcirc_ack_link[i]   = new sc_signal<bool>[DIRECTIONS];
        lcirc_buffer_full_status_link[i] = new sc_signal<TBufferFullStatus>[DIRECTIONS];
        lcirc_free_slots_link[i] = new sc_signal<int>[DIRECTIONS];
        lcirc_nop_link[i]   = new sc_signal<NoP_data>[DIRECTIONS];
    }
	std::cout << "Signals set" << std::endl;

    // Create tiles as a linear array (indexed by node id)
    core = new Tile*[N];

    // Dummy hub-related signals (LCirculant build does not use wireless hubs,
    // but Tile exposes hub_* ports that must be bound to avoid SystemC errors).
    // Сделаем их static, чтобы они жили всю симуляцию.
    static sc_signal<Flit> hub_dummy_flit("hub_dummy_flit");
    static sc_signal<bool> hub_dummy_bool("hub_dummy_bool");
    static sc_signal<TBufferFullStatus> hub_dummy_bufstatus("hub_dummy_bufstatus");
	std::cout << "Dummy set" << std::endl;

    for (int id = 0; id < N; id++) {
        char tile_name[64];
        sprintf(tile_name, "LCircTile_(#%d)", id);
        core[id] = new Tile(tile_name, id);

        // Configure router and processing element
        core[id]->r->configure(id,
                               GlobalParams::stats_warm_up_time,
                               GlobalParams::buffer_depth,
                               grtable);
        core[id]->r->power.configureRouter(GlobalParams::flit_size,
                                           GlobalParams::buffer_depth,
                                           GlobalParams::flit_size,
                                           string(GlobalParams::routing_algorithm),
                                           "default");

        core[id]->pe->local_id = id;

        if (GlobalParams::traffic_distribution == TRAFFIC_TABLE_BASED) {
            core[id]->pe->traffic_table = &gttable;
            core[id]->pe->never_transmit =
                (gttable.occurrencesAsSource(core[id]->pe->local_id) == 0);
        } else {
            core[id]->pe->never_transmit = false;
        }

        if (GlobalParams::traffic_distribution == TRAFFIC_HARDCODED)
            core[id]->pe->traffic_hardcoded = &ghtable;

        // Map clock and reset
        core[id]->clock(clock);
        core[id]->reset(reset);

        // Bind all hub-related ports to dummy signals so that SystemC ports are fully bound.
        // RX from hub (tile as sink)
        core[id]->hub_flit_rx(hub_dummy_flit);           // sc_in
        core[id]->hub_req_rx(hub_dummy_bool);            // sc_in
        core[id]->hub_ack_rx(hub_dummy_bool);            // sc_out
        core[id]->hub_buffer_full_status_rx(hub_dummy_bufstatus); // sc_out

        // TX to hub (tile as source)
        core[id]->hub_flit_tx(hub_dummy_flit);           // sc_out
        core[id]->hub_req_tx(hub_dummy_bool);            // sc_out
        core[id]->hub_ack_tx(hub_dummy_bool);            // sc_in
        core[id]->hub_buffer_full_status_tx(hub_dummy_bufstatus); // sc_in
    }

    // Helper lambda to connect a directed channel from src to dst
    auto connect_channel = [this](int src, int dst, int dir_src, int dir_dst) {
        // Use src-id/direction indexed signals for this directed channel
        sc_signal<Flit> &fl  = lcirc_flit_link[src][dir_src];
        sc_signal<bool> &rq  = lcirc_req_link[src][dir_src];
        sc_signal<bool> &ak  = lcirc_ack_link[src][dir_src];
        sc_signal<TBufferFullStatus> &bf = lcirc_buffer_full_status_link[src][dir_src];
        sc_signal<int> &fs   = lcirc_free_slots_link[src][dir_src];
        sc_signal<NoP_data> &np = lcirc_nop_link[src][dir_src];

        // TX side (src)
        core[src]->flit_tx[dir_src](fl);
        core[src]->req_tx[dir_src](rq);
        core[src]->ack_tx[dir_src](ak);
        core[src]->buffer_full_status_tx[dir_src](bf);
        core[src]->free_slots[dir_src](fs);
        core[src]->NoP_data_out[dir_src](np);

        // RX side (dst)
        core[dst]->flit_rx[dir_dst](fl);
        core[dst]->req_rx[dir_dst](rq);
        core[dst]->ack_rx[dir_dst](ak);
        core[dst]->buffer_full_status_rx[dir_dst](bf);
        core[dst]->free_slots_neighbor[dir_dst](fs);
        core[dst]->NoP_data_in[dir_dst](np);
    };

    // For each node, connect four LCirculant links:
    //   NORTH  (0) -> S1_MINUS
    //   SOUTH  (2) -> S1_PLUS
    //   EAST   (1) -> S2_PLUS
    //   WEST   (3) -> S2_MINUS
    for (int id = 0; id < N; id++) {
        int id_s1_plus  = (id + s1) % N;
        int id_s1_minus = (id - s1 + N) % N;
        int id_s2_plus  = (id + s2) % N;
        int id_s2_minus = (id - s2 + N) % N;

        // Extra safety checks – should never trigger if parameters are correct
        assert(id_s1_plus  >= 0 && id_s1_plus  < N);
        assert(id_s1_minus >= 0 && id_s1_minus < N);
        assert(id_s2_plus  >= 0 && id_s2_plus  < N);
        assert(id_s2_minus >= 0 && id_s2_minus < N);

        // S1_PLUS: src=id, dst=id_s1_plus
        connect_channel(id, id_s1_plus, DIRECTION_SOUTH, DIRECTION_NORTH);
        // S1_MINUS: src=id, dst=id_s1_minus
        connect_channel(id, id_s1_minus, DIRECTION_NORTH, DIRECTION_SOUTH);
        // S2_PLUS: src=id, dst=id_s2_plus
        connect_channel(id, id_s2_plus, DIRECTION_EAST, DIRECTION_WEST);
        // S2_MINUS: src=id, dst=id_s2_minus
        connect_channel(id, id_s2_minus, DIRECTION_WEST, DIRECTION_EAST);
    }
}
