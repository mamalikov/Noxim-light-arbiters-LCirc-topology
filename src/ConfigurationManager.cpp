...
void loadConfiguration() {
    ...
    // LCirculant network params (S in [1,6], d, k) and derived values
    if (GlobalParams::topology == TOPOLOGY_LCIRCULANT) {
        GlobalParams::lcirc_S = readParam<int>(config, "lcirc_S");
        GlobalParams::lcirc_d = readParam<int>(config, "lcirc_d");
        GlobalParams::lcirc_k = readParam<int>(config, "lcirc_k");

        // Compute N, s1, s2, a, b according to the selected LCirculant family
        int S = GlobalParams::lcirc_S;
        int d = GlobalParams::lcirc_d;
        int k = GlobalParams::lcirc_k;
        ...
        case 5:
            ...
            break;
        case 6:
            // S6: N = 2d^2 + 2d − 2k^2 − 2k
            // s1,s2 = d − k, d + k + 1
            // a = d + k + 1, b = 2d − 2k
            N  = 2*d*d + 2*d - 2*k*k - 2*k;
            s1 = d - k;
            s2 = d + k + 1;
            a  = d + k + 1;
            b  = 2*d - 2*k;
            break;
        default:
            cerr << "Error: lcirc_S must be in [1,6] for LCirculant topology" << endl;
            exit(1);
        }

        if (N <= 0 || a <= 0 || b <= 0) {
            cerr << "Error: invalid LCirculant parameters (non-positive N/a/b)" << endl;
            exit(1);
        }

        if (a * b < N) {
            cerr << "Error: LCirculant parameters require a*b >= N (a="
                 << a << ", b=" << b << ", N=" << N << ")" << endl;
            exit(1);
        }

        GlobalParams::lcirc_N  = (int)N;
        GlobalParams::lcirc_s1 = (int)s1;
        GlobalParams::lcirc_s2 = (int)s2;
        GlobalParams::lcirc_a  = (int)a;
        GlobalParams::lcirc_b  = (int)b;

        // Pre-compute base vectors a*, b*, a*+b* once for this topology
        switch (S) {
        case 1:
            GlobalParams::lcirc_ax  = d + k - 2;
            GlobalParams::lcirc_ay  = d - k;
            GlobalParams::lcirc_bx  = -3;
            GlobalParams::lcirc_by  = 2*d - 2*k;
            break;
        case 2:
            GlobalParams::lcirc_ax  = d + k;
            GlobalParams::lcirc_ay  = d - k - 1;
            GlobalParams::lcirc_bx  = 0;
            GlobalParams::lcirc_by  = 2*d - 2*k - 1;
            break;
        case 3:
            GlobalParams::lcirc_ax  = d + k;
            GlobalParams::lcirc_ay  = k - d + 1;
            GlobalParams::lcirc_bx  = 0;
            GlobalParams::lcirc_by  = 2*d - 2*k;
            break;
        case 4:
            GlobalParams::lcirc_ax  = d - k;
            GlobalParams::lcirc_ay  = d + k;
            GlobalParams::lcirc_bx  = k - d;
            GlobalParams::lcirc_by  = d + k + 1;
            break;
        case 5:
            GlobalParams::lcirc_ax  = d + k;
            GlobalParams::lcirc_ay  = d - k;
        ...
        GlobalParams::lcirc_abx = GlobalParams::lcirc_ax + GlobalParams::lcirc_bx;
        GlobalParams::lcirc_aby = GlobalParams::lcirc_ay + GlobalParams::lcirc_by;

        // Number of tiles/routers in LCirculant (used by statistics code that
        // treats non-mesh topologies as "delta" and iterates over core[0..n_delta_tiles-1])
        GlobalParams::n_delta_tiles = GlobalParams::lcirc_N;

        // Reuse mesh_dim_x/mesh_dim_y for statistics / visualization
        GlobalParams::mesh_dim_x = GlobalParams::lcirc_a;
        GlobalParams::mesh_dim_y = GlobalParams::lcirc_b;
    }
...
void showHelp(char selfname[])
{
    cout << "Usage: " << selfname << " [options]" << endl
	 ...
	 << "\t-vc N\t\t\tNumber of virtual channels" << endl
         ...
         << "\t\tMESH\t\t2D Mesh" << endl
         << "\t\tLCIRCULANT\tLCirculant circulant topology" << endl
         ...
         << "\t\tHOPCOUNT_MAX\tPriority to packets with maximum hopcount" << endl
         << "\t\tHOPCOUNT_MIN\tPriority to packets with minimum hopcount" << endl
         << "\t\tDISTANCE_MIN\tPriority to packets with minimum remaining distance" << endl
         << "\t\tDISTANCE_MAX\tPriority to packets with maximum remaining distance" << endl
         ...
         << "\t-lcircS N\t\tLCirculant family index S in [1,6]" << endl
         << "\t-lcircd N\t\tLCirculant parameter d" << endl
         << "\t-lcirck N\t\tLCirculant parameter k" << endl
         << endl
...
{
	if (GlobalParams::topology==TOPOLOGY_MESH ||
        GlobalParams::topology==TOPOLOGY_LCIRCULANT)	{
...
void parseCmdLine(int arg_num, char *arg_vet[])
{
	...
	for (int i = 1; i < arg_num; i++) 
	{
	    ...
	    else if (!strcmp(arg_vet[i], "-lcircS"))
		GlobalParams::lcirc_S = atoi(arg_vet[++i]);
	    else if (!strcmp(arg_vet[i], "-lcircd"))
		GlobalParams::lcirc_d = atoi(arg_vet[++i]);
	    else if (!strcmp(arg_vet[i], "-lcirck"))
		GlobalParams::lcirc_k = atoi(arg_vet[++i]);
