...
SC_MODULE(NoC)
{
    ...
    // Signals for LCirculant topology (one entry per node and direction)
    // Directions are logically interpreted as:
    //   NORTH  -> S1_MINUS
    //   SOUTH  -> S1_PLUS
    //   WEST   -> S2_MINUS
    //   EAST   -> S2_PLUS
    sc_signal<Flit> **lcirc_flit_link;
    sc_signal<bool> **lcirc_req_link;
    sc_signal<bool> **lcirc_ack_link;
    sc_signal<TBufferFullStatus> **lcirc_buffer_full_status_link;
    sc_signal<int> **lcirc_free_slots_link;
    sc_signal<NoP_data> **lcirc_nop_link;
	...
	if (GlobalParams::topology == TOPOLOGY_MESH)
	    // Build the Mesh
	    buildMesh();
	else if (GlobalParams::topology == TOPOLOGY_LCIRCULANT)
	    buildLCirculant();
