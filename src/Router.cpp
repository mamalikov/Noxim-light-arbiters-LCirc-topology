...
void Router::txProcess()
{
  ...
  if (reset.read()) 
      ...
      for (int j = 0; j < DIRECTIONS + 2; j++) 
	{
	  ...
	  {
	      // Use selection strategy arbitration instead of random
	      int arb_idx = selectionStrategy->arbitrate(this, reservations);
	      assert(arb_idx >= 0 && arb_idx < (int)reservations.size());

	      int o = reservations[arb_idx].first;
	      int vc = reservations[arb_idx].second;
	     // LOG<< "found reservation from input= " << i << "_to output= "<<o<<endl;
		       ...
		       (buffer_full_status_tx[o].read().mask[vc] == false) ) 
		  {
		      ...
		      // Increment hopcount if forwarding to another router (not local destination)
